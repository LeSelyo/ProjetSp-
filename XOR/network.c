...
void random_test(Matrix *hw, Matrix *hb, Matrix *ow, Matrix *ob)
{
    ...
    {
        double i1 = ipt_random_XOR();
        double i2 = ipt_random_XOR();
...
int main()
{
    ...
    for (int i = 0; i < (&hidden_weights)->rows; i++)
    {
        ...
        {
            m_setIndex(&hidden_weights, i, j, random_weight());
        }
    }
    ...
    for (int i = 0; i < (&hidden_bias)->rows; i++)
    {
        ...
        {
            m_setIndex(&hidden_bias, i, j, random_weight());
        }
    }
    ...
    for (int i = 0; i < (&output_weights)->rows; i++)
    {
        ...
        {
            m_setIndex(&output_weights, i, j, random_weight());
        }
    }

    Matrix output_bias;
    m_init(&output_bias, 1, num_outputs);
    m_setIndex(&output_bias, 0, 0, random_weight());
